...
## Уточнение после проверки V7 path

Важно: этот монитор проверяет egress-направление с сервера, а не полный путь `phone -> V7 -> egress`.

Поэтому его результат нельзя автоматически трактовать как "канал плохой". Он показывает только, что конкретный сервис через конкретный egress из V7 выглядит проблемно.

Отдельно добавляется `v7-path-sanity-check`, который проверяет именно V7-слой:
...
- direct leak guard;
- user policy routes;
- наличие признаков, что V7 сам может обрубать или замедлять трафик.

После проблемы с Telegram правильная диагностика такая:

```text
service matrix = сервис/egress виден или нет
path sanity    = не ломает ли V7 путь клиента
```

Для активного телефона `10.0.0.3` сейчас выбран `vless`, а V7-layer защищён MSS clamp, поэтому Telegram работает хорошо.
