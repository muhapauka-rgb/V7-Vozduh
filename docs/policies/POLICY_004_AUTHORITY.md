...
| CS5: break-glass authority is exceptional and audited. | `UNKNOWN` | OMP, operator authority. | Explicit break-glass policy is not implemented or certified. | Reuse OMP authority boundary if needed. | `DOCUMENTATION_ONLY`: decide in V7 fit analysis. |
...
| Break-glass authority. | `PARTIAL` | `ADAPT` | It should exist only as exceptional audited operator policy, not runtime default. | OMP, operator authority. | Define break-glass as documented exception path. | `SMALL_EXTENSION` | Medium: incident resilience. | `C3` |
