...
| CS6: perfect freshness impossible; bounded staleness or explicit stop required. | `PARTIALLY_IMPLEMENTED` | Freshness actionability, OMP stop rules. | Explicit stop exists; bounded staleness policy is not fully defined. | Reuse stop conditions. | `CONFIGURATION_ONLY`: decide stale allowance by class. |
...
| Perfect freshness impossible; bounded stop required. | `YES` | `REUSE` | Explicit stops exist; bounded staleness needs class policy. | Freshness actionability, OMP stop rules. | Decide stale allowance by class. | `NONE` | Medium high: practical reliability. | `C6` |
