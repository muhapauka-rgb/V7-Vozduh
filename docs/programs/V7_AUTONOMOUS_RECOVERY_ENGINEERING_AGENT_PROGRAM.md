...
### ОБЯЗАТЕЛЬНОЕ УСЛОВИЕ MISSION — ОПТИМИЗАЦИЯ ЗАТРОНУТОГО КОДА БЕЗ ЦИКЛОВ ПЕРЕПРОВЕРКИ

ЦЕЛЬ

Полное переключение клиентов ≤7 секунд, ориентир — 3 секунды, и массовая сертификация/переключение без искусственного ограничения количества клиентов, в пределах доказанной мощности оборудования. Оптимизация кода должна помогать достижению этой цели.

1. СНАЧАЛА ПРОВЕРЬ СУЩЕСТВУЮЩЕЕ

Перед добавлением или изменением механизма проверь наличие подходящей реализации. Переиспользуй, исправь или расширь существующий механизм. Добавляй новое только при подтверждённом отсутствии необходимой возможности.

2. АНАЛИЗИРУЙ ЗАТРОНУТУЮ ЦЕПОЧКУ

При существенном изменении проследи вызывающий код, исполнителей, потребителей результата, состояния и защитные проверки. Расширяй область анализа только по конкретной обнаруженной зависимости или риску. Повторный полный аудит проекта на каждом шаге запрещён.

3. УСТРАНЯЙ ДОКАЗАННО ЛИШНЕЕ

Выявляй дубликаты, устаревшие ветки, исторические остатки, незавершённые механизмы и выполняемые действия, которые не добавляют необходимого продуктового результата или факта безопасности. Проверяй, не сделало ли новое решение прежнюю цепочку избыточной.

Доказанно лишнее удаляй или упрощай в рамках текущего изменения. Не оставляй старую и новую реализации одновременно без необходимой совместимости или ограниченного переходного периода.

4. СОХРАНЯЙ НЕОБХОДИМОЕ ПОВЕДЕНИЕ

Для удаления проверь фактическое использование и сравни поведение с механизмом и без него. Сохраняй действующих владельцев ответственности, Authority, изоляцию, актуальность данных, защиту от повторов, rollback, маршрутизационную истину и обязательную проверку сервисов/S11.

Прохождение тестов само по себе не доказывает ненужность механизма. Незавершённый код не считай ненужным автоматически.

5. КАЖДАЯ ПРОВЕРКА ДОЛЖНА ВЕСТИ К РЕШЕНИЮ

Перед проверкой определи:
— какой конкретный вопрос или риск она разрешает;
— какой результат достаточен;
— какое действие последует при успехе и при неуспехе.

Группируй связанные проверки в один достаточный набор для целостного изменения. Не создавай отдельный цикл согласования и отчёт для каждой мелкой проверки.

После получения достаточного результата переходи к исправлению, интеграции или полному продуктовому опыту. Не назначай дополнительную проверку только «для уверенности».

6. ПОВТОРЯЙ ТОЛЬКО ПО ПРИЧИНЕ

Используй уже полученные доказательства, пока относящиеся к ним код, условия и исходные данные не изменились.

Повтор или расширение проверки допустимы при:
— изменении поведения или значимых входных данных;
— новом отказе либо противоречии;
— выявленном непокрытом риске;
— истечении обязательной свежести runtime-доказательств.

Причину повтора указывай кратко. Обязательные текущие проверки перед изменением маршрутов остаются обязательными.

7. ПРЕРЫВАЙ ПОВТОРЕНИЕ ОДНОГО ТУПИКА

Если одна и та же ошибка повторилась дважды без новых данных или сужения причины, третий одинаковый запуск запрещён.

Перейди к анализу конкретного несовпадения между производителем и потребителем данных, исправь причину либо направь в «Агент Времени» факты и вопрос, требующий решения. Повторный запуск разрешён после содержательного изменения или появления новых доказательств.

Это правило меняет способ работы, а не разрешает пропустить защиту или завершить Mission.

8. НЕОБЯЗАТЕЛЬНАЯ ОПТИМИЗАЦИЯ НЕ БЛОКИРУЕТ ЦЕЛЬ

Если ненужность механизма не доказана в пределах текущего изменения — сохрани его, кратко зафиксируй неопределённость и продолжай Mission.

Если неопределённость касается безопасности необходимого действия, приостанови только зависимое действие, направь вопрос в «Агент Времени» и продолжай независимую разрешённую работу.

9. ЗАВЕРШАЙ ПО РЕЗУЛЬТАТУ MISSION

Тест, локальная правка, отчёт, commit или deploy не завершают Mission. Продолжай до согласованного продуктового результата либо конкретной внешней границы, требующей решения.

Не создавай новые архитектурные слои ради упрощения файлов и не задерживай результат ради сокращения строк.

В итоговом отчёте кратко укажи: достигнутый результат, что переиспользовано, что упрощено или удалено, чем подтверждена безопасность и какие ограничения остались.

### Current Polygon ingress decision

The current approved Polygon extension may use one real L3 client-ingress
container/internal network in addition to router/backend. Bind the actual new
topology to a fresh lab Authority decision. A veth ingress named `wg0` is an
explicit L3 adapter, not WireGuard protocol evidence. If the required path
depends on WG keys/handshake/encapsulation/reconnect, use actual isolated WG
instead of weakening that predicate. Five IP identities in one client container
are logical clients, not independent devices or a hardware-scale proof. Missing
success receipts never establish zero effect; capture partial state before
teardown and distinguish owner rollback from disposable-container deletion.
