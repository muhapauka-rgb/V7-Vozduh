...
| `CONTROLLED_RUN_PRIMARY_STOP` | `APPROVED_PACKET_INVALIDATED_BY_SOURCE_DRIFT` |
| `CONTROLLED_RUN_RESPONSIBILITY_CLASS` | `FRESH_EVIDENCE_REVALIDATION_REQUIRED` |
| `CONTROLLED_RUN_AUTHORITY_REQUIRED_NOW` | `NO; no valid packet exists` |
...
| `current_primary_stop` | `APPROVED_PACKET_INVALIDATED_BY_SOURCE_DRIFT` |
...
| Stop | Current use |
| --- | --- |
| `APPROVED_PACKET_INVALIDATED_BY_SOURCE_DRIFT` | Active U01 approval is terminally invalid for the old packet; fresh Phase 4A is required before any new Authority request. |
...
| `CAP-CON-06` | Controlled Run responsibility | Initial Phase 4A report says `INTENT_BLOCKED_BY_ENGINEERING_AUTHORITY`; later implementation and certification closed the contract; first exact approval then invalidated on fresh source drift | CPS/OMP current state | historical values `SUPERSEDED`; current boundary is `APPROVED_PACKET_INVALIDATED_BY_SOURCE_DRIFT`, intent remains open and next action is fresh Phase 4A |
