...
For service-matrix observations the existing lifecycle binding is explicit: `tools/v7-service-matrix-test` preserves a source/service-bound episode, `tools/v7-service-matrix-refresh-all` runs the existing capture-only consumer after the batch, and `tools/v7-users-autoswitch --consume-passive-events-only` records the existing Situation/Trace/STOP_SAFE/temporal/replay/Learning capture. The continuity window must cover the normal 15-minute timer, randomized delay and batch runtime while still splitting unrelated long gaps. Consumer absence or failure must be materialized in the same matrix summary/event as `V7_PASSIVE_SERVICE_EVENT_CONSUMER_REPAIR`; it must never degrade into notification-only failure or silently grant natural provenance.
...
The production Matrix lifecycle must consume these events automatically through the existing capture-only autoswitch owner. That consumer must reject every execution, Packet, lease, rollback and Authority input, expose zero forbidden effects, and publish an exact OMP repair frontier in the existing Matrix artifact whenever liveness is broken. The producer's continuity interval must be derived from the real observation cadence rather than a shorter nominal timer period; otherwise a continuing outage is illegally fragmented and cannot reach its consumer.
