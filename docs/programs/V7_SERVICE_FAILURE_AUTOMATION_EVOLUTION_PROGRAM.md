...
| `N10` | Bounded ordinary rollout only after N8/N9: controlled -> one ordinary-like case -> small cohort -> bounded production, with rollback and no manufactured ordinary failure. Before any new bounded-production move under Core-primary routing, the existing route/projection owner must prove one atomic affected-cohort projection commit after canonical cohort assignments, with exact affected-scope and whole-system verification. A per-user full-population rebuild is not an N10 admission. |
