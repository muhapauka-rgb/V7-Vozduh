...
1. Use `tools/v7-governed-canary-dry-run-cycle` as the single read-only knowledge-gated governed canary preparation entrypoint before any approval/apply discussion.
2. Use the knowledge-gated `operator_decision_surface` before any packet/governed execution preview.
3. Use `tools/v7-autonomy-trust-evidence-inventory --routing-foundation-only` before any routing/autonomy escalation.
4. Use the decision-to-outcome-to-learning integration to classify real governed/manual outcomes as outcome quality, learning records, knowledge growth, and decision effectiveness.
5. Generate the next real outcome through a single governed canary packet using existing packet, restore barrier, feedback, and learning owners; do not synthesize missing closure fields.
6. Continue service/channel real probe cycles through existing service, quality, prediction, feedback, and closure owners.
7. Run `AUTONOMY.CANDIDATE_OUTCOME.GOVERNED_REALITY_GENERATION` through existing candidate/outcome owners after packet readiness is clean.
