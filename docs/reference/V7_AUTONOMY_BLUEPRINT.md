...
3. Use the decision-to-outcome-to-learning integration to classify real governed/manual outcomes as outcome quality, learning records, knowledge growth, and decision effectiveness.
4. Generate the next real outcome through a single governed canary packet using existing packet, restore barrier, feedback, and learning owners; do not synthesize missing closure fields.
5. Continue service/channel real probe cycles through existing service, quality, prediction, feedback, and closure owners.
6. Run `AUTONOMY.CANDIDATE_OUTCOME.GOVERNED_REALITY_GENERATION` through existing candidate/outcome owners after packet readiness is clean.
7. Collect time-separated prediction forecast -> later actual evidence until forecast source confidence rises.
8. Collect contextual operator comparisons only where the operator has enough context.
