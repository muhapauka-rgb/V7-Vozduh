...
1. Use `tools/v7-autonomy-trust-evidence-inventory` `suitability_quality_model`, `suitability_knowledge_growth`, and `autonomy_grade_suitability_program` to decide which real candidate/suitability outcome is worth collecting next.
2. Use `tools/v7-governed-canary-dry-run-cycle` as the single read-only knowledge-gated governed canary preparation entrypoint before any approval/apply discussion.
3. Use the knowledge-gated `operator_decision_surface` before any packet/governed execution preview.
4. Use `tools/v7-autonomy-trust-evidence-inventory --routing-foundation-only` before any routing/autonomy escalation.
5. Use the decision-to-outcome-to-learning integration to classify real governed/manual outcomes as outcome quality, learning records, knowledge growth, and decision effectiveness.
6. Generate real candidate/suitability outcomes through governed/manual paths only when explicitly approved; do not synthesize missing closure fields.
7. Continue service/channel real probe cycles through existing service, quality, prediction, feedback, and closure owners.
8. Collect time-separated prediction forecast -> later actual evidence until forecast source confidence rises.
