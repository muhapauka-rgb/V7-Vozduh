...
- Known caveats: Continuous production autonomy daemon is not active as of POOL.3/EVENT.1/AUTONOMY.ROOT. Truth says `autoswitch_scheduler_active=false` and `autoswitch_service_active=false`. EVENT.1 proved the current read-only chain can preview planner/packet/restore/rollback/feedback/learning surfaces but must stop because confidence/trust/prediction floors fail, operator comparison evidence is below floor, restore barrier readiness is blocked, and no live event consumer is certified. AUTONOMY.ROOT clarified that BA evidence is consumed and raises governed inherited execution trust, but does not close operator-free autonomy trust. AUTONOMY.EVIDENCE.REAL_SOURCE_CONFIDENCE_COLLECTION deployed source-confidence attribution in the existing read-only trust inventory. Current production verdict is `EVIDENCE_MIXED`: prediction matches are complete (`21/21`, `0` pending), blast and rollback evidence are sufficient (`100.0` each), but service evidence is low-confidence (`0.39` mean row confidence), candidate outcomes are incomplete (`83/156`), and operator comparisons are `0`. Current floors remain below canary target: confidence `39.042`, trust `54.282`, prediction confidence `35.486`, operator earned confidence `45.862`.
