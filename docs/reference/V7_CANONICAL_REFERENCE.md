...
- Known caveats: This is an evolution/readiness surface, not a controller. It can show `AUTHORITY_BOUNDARY`, `REAL_GAP`, or `READY_FOR_TIER_2_GOVERNED_REVIEW`, but it cannot approve apply. Local workspace without production runtime state can show zero floor values; production inventory is required for runtime-grade numbers.
...
- Last verified commit: `pending V7.AUTONOMOUS_ROUTING_EVOLUTION_PROGRAM commit`.
