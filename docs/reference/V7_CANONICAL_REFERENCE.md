...
- Where it is displayed: Primary Channel table column and Channel Drawer first screen. The Channel Drawer first screen is Decision-first: Channel → Decision → Reason → Signals → Problems → Engineer Diagnostics entry, with no score or technical health above the decision.
...
- Related reports / ADRs: `CHANNELS_FINAL_DENSITY_AND_CONSISTENCY_REPORT.md`, `CHANNELS_OPERATOR_ENGINEER_SEPARATION_REPORT.md`, ADR-006, ADR-007, ADR-010.
