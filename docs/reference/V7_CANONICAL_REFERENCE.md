...
4. Prediction actuals are built by `admin_core/intelligence_workers.py::build_prediction_actual_rows` from existing service/channel score rows and bounded decision records.
...
11. Raising prediction confidence must use existing evidence owners only: repeated real forecast-to-later-actual comparisons, fresher service/quality/trust/blast inputs, existing snapshot refresh, and existing shadow/operator comparison evidence.
...
13. Next safe phase: `AUTONOMY.PREDICTION.EVIDENCE.2_REAL_OUTCOME_CONFIDENCE_COLLECTION`.
14. Last verified commit: `7695fc38e1dfd7fc577671bd8a27b16b06cf4321`.
