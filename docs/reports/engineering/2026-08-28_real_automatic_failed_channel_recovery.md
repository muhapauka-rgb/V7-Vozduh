...
- после операции повторный вызов существующего consumer корректно вернул
  `STOP_SAFE_CURRENT_INCIDENT_NOT_ACTIONABLE`: повторное движение не создаётся.
...
Дополнительная revalidation после публикации отчёта:

- объединённый набор текущих unit/regression тестов: `442 PASS`;
- `v7-health.service` фактически работает в режиме `--role-based-fast` и
  содержит вызов существующего `v7-service-matrix-refresh-all` consumer;
- отдельного старого Matrix/Telegram timer в systemd нет;
- в текущем registry активных обычных клиентов на `vless` не осталось
  (`10.7.0.7` отключён), поэтому искусственная вторая production-миграция не
  создавалась.
