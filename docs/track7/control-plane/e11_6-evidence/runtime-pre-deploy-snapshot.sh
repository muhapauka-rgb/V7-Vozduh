...
echo "# E11.6 runtime pre-deploy safety snapshot"
echo "## date -u"
date -u

echo
echo "## runtime diagnose tool"
ls -l /usr/local/bin/v7-egress-diagnose 2>&1
stat -c '%U:%G %a %s %y %n' /usr/local/bin/v7-egress-diagnose 2>&1
sha256sum /usr/local/bin/v7-egress-diagnose 2>&1
bash -n /usr/local/bin/v7-egress-diagnose
echo "runtime_bash_n_rc=$?"

echo
echo "## runtime diagnose command path excerpts"
grep -nE 'awg show|wg show|latest-handshakes|curl_ok_but_handshake_stale' /usr/local/bin/v7-egress-diagnose 2>&1
...
echo
echo "## wireguard registry truth"
grep -n 'wireguard-1779454504-c43409' /opt/v7/egress/state/egress.registry 2>&1
grep -n 'current=wireguard-1779454504-c43409' /opt/v7/egress/state/users.registry 2>&1 || true
sha256sum /opt/v7/egress/state/users.registry /opt/v7/egress/state/egress.registry 2>&1

echo
echo "## focused state entries"
for f in \
...
echo
echo "## local-runtime target readiness if installed"
if command -v v7-second-canary-target-readiness >/dev/null 2>&1; then
  v7-second-canary-target-readiness --pretty 2>&1
else
  echo "v7-second-canary-target-readiness not installed on runtime"
fi
