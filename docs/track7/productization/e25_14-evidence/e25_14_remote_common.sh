...
PACKET="${PACKET:-/tmp/e25_14_packet.json}"
...
candidate_row() {
  grep "ip=$CAND " "$STATE/users.registry" || true
}

candidate_field() {
  key="$1"
  awk -v ip="$CAND" -v key="$key" '$0 ~ "ip=" ip {
    for(i=1;i<=NF;i++){split($i,kv,"="); if(kv[1]==key){print kv[2]; exit}}
...
current_users_for() {
  target="$1"
  awk -v target="$target" '/enabled=1/ && $0 ~ "current=" target {
    for(i=1;i<=NF;i++){split($i,kv,"="); if(kv[1]=="ip"){print kv[2]}}
  }' "$STATE/users.registry"
}

target_users_count() {
  current_users_for "$TARGET" | wc -l | tr -d " "
}
...
hidden_movers() {
  pgrep -af 'v7-user-switch|v7-routing-sync|v7-users-autoswitch.*--apply' 2>/dev/null |
    grep -v -E 'pgrep|grep|e25_14_|ssh v7-vps' || true
}

hidden_movers_count() {
  matches="$(hidden_movers)"
...
route_table_1009() {
  ip route show table 1009 | tr "\n" " "
}

route_get_candidate() {
  ip route get 8.8.8.8 from "$CAND" iif wg0 2>/dev/null | tr "\n" " "
}
...
audit_tail() {
  if [ -f "$AUDIT" ]; then tail -n 8 "$AUDIT"; else echo "audit_store_missing=$AUDIT"; fi
}
...
payload=json.load(open(sys.argv[1], encoding="utf-8"))
expected=payload.pop("packet_hash", "")
