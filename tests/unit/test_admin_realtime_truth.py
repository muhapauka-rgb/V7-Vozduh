...
class AdminRealtimeTruthTest(unittest.TestCase):
    ...
    def test_inline_channel_choice_starts_governed_rebind_without_intermediate_drawer(self):
        ...
        self.assertIn("USER_SWITCH_ADMISSION_WAIT_MS = 45000", inline)
        self.assertIn("requestOperatorProfileEgressRebind(ip, egress, Date.now() + USER_SWITCH_ADMISSION_WAIT_MS)", inline)
        self.assertIn("operator_profile_rebind_admission_wait_exceeded", inline)
        self.assertIn("после его освобождения маршрут подтверждается максимум за 7 секунд", inline)
