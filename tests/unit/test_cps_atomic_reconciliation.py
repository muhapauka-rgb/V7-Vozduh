...
class CpsAtomicReconciliationTest(unittest.TestCase):
    ...
    def test_02_operational_authority_with_stop_safe_projection_fails(self):
        drift = self.cps.replace("| `current_primary_stop` | `REAL_WORLD_LIMIT_CAPABILITY_LOCAL; global program frontier is AEP_PHASE_6_PRODUCTION_CERTIFICATION_PREPARATION` |", "| `current_primary_stop` | `STOP_SAFE` |", 1)
    ...
    def test_09_sequence_position_one_stop_differs_fails(self):
        row = next(line for line in self.cps.splitlines() if line.startswith("| `1` | `AEP_PHASE_6_READY` program frontier"))
    ...
    def test_14_post_write_reread_detects_partial_update_and_rolls_back(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            def corrupt(written):
                ...
                written.write_text(text.replace("| `current_primary_stop` | `REAL_WORLD_LIMIT_CAPABILITY_LOCAL; global program frontier is AEP_PHASE_6_PRODUCTION_CERTIFICATION_PREPARATION` |", "| `current_primary_stop` | `STOP_SAFE` |", 1), encoding="utf-8")
