...
class CpsTerminalMissionIdentityRolesTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.latest = "CAP-U01_FIRST_REAL_GOVERNED_OUTCOME_CLOSURE_V2"
        cls.previous = "V7_OMP_CPS_DELEGATED_POLICY_LIVE_STATE_RECONCILIATION_V1"
    ...
    def temp_root_with_report(self, first, second):
        ...
        root = Path(tmp.name)
        report = root / "docs/reports/engineering/2026-07-12_105740_first_real_governed_outcome_closure_v2.md"
    ...
    def test_09_header_timestamp_predates_latest_start_fails(self):
        drift = self.cps.replace("State captured: 2026-07-12T10:57:40+0700", "State captured: 2026-07-12T02:00:00+0700", 1)
    ...
    def test_10_latest_report_header_id_mismatch_fails(self):
        tmp, root = self.temp_root_with_report("Mission ID: `OLD`", "Run Nonce: `V7_CAP_U01_OUTCOME_V2_0CBFD6C64A8B`")
        try:
    ...
    def test_19_operational_state_is_bounded_policy_active(self):
        ...
        self.assertEqual(live["CURRENT_STOP_CONDITION"].strip("`"), "POST_FIX_FRESH_TRANSACTION_REQUIRED")
    ...
    def test_22_current_stop_is_live_reality_only(self):
        self.assertEqual(self.lib.cps_live_state_consistency(self.cps, root=ROOT, omp_text=self.omp)["current_stop"], "POST_FIX_FRESH_TRANSACTION_REQUIRED")
