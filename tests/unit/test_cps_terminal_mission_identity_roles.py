...
class CpsTerminalMissionIdentityRolesTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.latest = "V7_AEP_PHASE_3_CERTIFIED_AUTONOMOUS_BEHAVIOUR_GAP_REGISTER_V1"
        cls.previous = "V7_AEP_PHASE_2_INDEPENDENT_ACCEPTANCE_AND_LOCK_V1"
    ...
    def temp_root_with_report(self, first, second):
        ...
        root = Path(tmp.name)
        report = root / "docs/reports/engineering/2026-07-14_103725_aep_phase_3_to_omp_mission_generation.md"
    ...
    def test_09_header_timestamp_predates_latest_start_fails(self):
        drift = self.cps.replace("State captured: 2026-07-14T10:37:25+0700", "State captured: 2026-07-12T02:00:00+0700", 1)
    ...
    def test_10_latest_report_header_id_mismatch_fails(self):
        tmp, root = self.temp_root_with_report("Mission ID: `OLD`", "Run Nonce: `V7_AEP_PHASE_3_GAP_REGISTER_V1_4C9E71A25B8D`")
        try:
    ...
    def test_12_omp_latest_closure_pointer_mismatch_fails(self):
        drift = self.omp.replace("docs/reports/engineering/2026-07-14_103725_aep_phase_3_to_omp_mission_generation.md", "docs/reports/engineering/stale.md")
    ...
    def test_19_operational_state_is_bounded_policy_active(self):
        ...
        self.assertEqual(live["CURRENT_ACTIVE_SCOPE"].strip("`"), "AEP_PHASE_3_GAP_REGISTER_INDEPENDENT_ACCEPTANCE")
    ...
    def test_21_cap_u01_complete_and_u07_is_sequence_position_one(self):
        ...
        self.assertIn("`AEP_PHASE_3_READY_FOR_ACCEPTANCE` program frontier", result["sequence_position_1"])
