...
class GovernedCanaryCliTest(unittest.TestCase):
    ...
    def test_initial_certification_source_binding_uses_only_existing_route_writer(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (state / "egress.registry").write_text(
                "id=source interface=v7execwg0 enabled=1 controlled_certification_source=1 controlled_source_reservation_id=ctres-test\n",
                encoding="utf-8",
            )
    ...
    def test_initial_certification_source_binding_rebinds_only_the_exact_identity(self):
        module = load_cli_module()
        with tempfile.TemporaryDirectory() as tmp:
            state = Path(tmp)
            (state / "users.registry").write_text(
                "ip=10.7.0.124 current=prior-source table=1122 enabled=1 certification_user=1 certification_group=group\n",
                encoding="utf-8",
            )
            (state / "egress.registry").write_text(
                "id=source interface=v7execwg0 enabled=1 controlled_certification_source=1 controlled_source_reservation_id=ctres-test\n",
            ...
            with mock.patch.object(
                module, "scoped_user_route_check",
                return_value={"passed": True, "reason": "verified"},
            ), mock.patch.object(
                module.operator_execution, "build_autonomous_execution_control_state",
                return_value={"generation": "aec_unit"},
            ), mock.patch.object(module.operator_execution, "write_json_atomic"), mock.patch.object(
                module.operator_execution, "autonomous_execution_control_decision",
                return_value={"allowed_forward_mutation": True},
            ), mock.patch.object(
                module.operator_execution, "finalize_autonomous_execution_control_window",
        ...
        self.assertTrue(result["ok"], result)
        self.assertEqual(result["binding"]["prior_source"], "prior-source")
    ...
    def test_availability_first_stage_fails_closed_before_any_effect_without_policy(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (state / "users.registry").write_text(
                (
                    "ip=10.7.0.1 current=vless enabled=1 "
            ...
            before = (state / "users.registry").read_text(
                encoding="utf-8"
            )
            with mock.patch.object(
                module,
                ...
                return_value={
                    ...
                    "diagnostic": {
                        "status": (
                            ...
                            "AVAILABILITY_FIRST_CONTRACT_REQUIRED"
                        ),
                        "availability_first_standing_policy_admission": {
                            "ok": False,
                            "blockers": [
                                "availability_first_policy_not_active"
                            ],
                        },
                    },
                    "blockers": [],
            ...
            after = (state / "users.registry").read_text(
                encoding="utf-8"
            )
        self.assertEqual(
            result["stop_reason"],
    ...
    def test_availability_first_subtransaction_reuses_standing_and_substrate_owners(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            args = argparse.Namespace(
                ...
                max_users=2,
                expected_standing_policy_contract_id=(
                    activated["contract"]["contract_id"]
                ),
                expected_standing_policy_contract_hash=(
                    activated["contract"]["contract_hash"]
                ),
                state_dir=str(state),
                approved_source="vless",
                controlled_certification_campaign_request_id="cpsauth",
                ...
                _availability_first_stage_request={
                    "stage": 5,
                    "target_id": "awg3",
                    "expected_users": ["10.7.0.1", "10.7.0.2"],
        ...
        self.assertEqual(
            result["delegated_policy_authority"]
    ...
    def test_ct_m0f_reset_reconciles_terminal_sample_route_residue(self):
        ...
        args = argparse.Namespace(
            ct_m0f_standing_validation_reservation_id=(
        ...
        with mock.patch.object(
            ...
            side_effect=lambda path, default=None: (
                ...
                if str(path).endswith("policy.json")
                else {"items": {"execution-source": {"services": {
                    "google": {"status": "OK", "ok": True},
                }}}}
            ),
        ), mock.patch.object(
            module.operator_execution,
            ...
            side_effect=lambda path: (
                [{"ip": reservation["user"], "current": "awg3"}]
                if str(path).endswith("users.registry")
                else [{"id": "execution-source", "enabled": "1"}]
            ),
        ), mock.patch.object(
            module,
            "execute_governed_transaction_with_guards",
            ...
            module.operator_execution,
            "record_ct_m0f_standing_validation_sample_terminal",
        ) as record_terminal, mock.patch.object(
            module.operator_execution,
            "ct_m0f_standing_validation_budget_status",
            return_value={"campaign_complete": False},
        ):
        ...
        self.assertEqual(result["sample_terminal"], terminal)
        self.assertEqual(result["users_moved"], 1)
    ...
    def test_ct_m0f_reset_restores_owner_disabled_source_without_return_move(self):
        module = load_cli_module()
        reservation = {
            "reservation_id": "ctm0fsample_controlled",
            "contract_id": "ctm0fsdpc_test",
            "contract_hash": "h" * 64,
            "user": "10.7.0.107",
            "source": "execution-source",
        ...
        with mock.patch.object(
            ...
            module.subprocess,
            "run",
            return_value=mock.Mock(returncode=0, stdout="restored"),
    ...
    def test_controlled_setup_admits_only_historical_dedicated_identity_and_empty_source(self):
        ...
        self.assertIn(
            "identity_not_in_durable_legacy_certification_pool",
            ordinary["blockers"],
        )
    ...
    def test_controlled_topology_reuses_registry_marked_certification_identity(self):
        ...
        unmarked = module.controlled_certification_setup_selection(
            users=[{
                "ip": "10.7.0.100",
                "current": "1",
                "enabled": "1",
        ...
        self.assertEqual(unmarked["selection_status"], "STOP_SAFE")
        self.assertIn(
        ...
        continuation = module.controlled_certification_setup_selection(
            users=[
                {
                    "ip": "10.7.0.100",
                    "current": "1",
            ...
            source="vless",
            certification_identity_mode="REGISTRY_MARKED",
            allow_same_campaign_occupants=True,
        )
        self.assertEqual(
            continuation["selection_status"],
            "SELECTED",
        )
        self.assertTrue(continuation["same_campaign_continuation"])
    ...
    def test_one_user_substrate_preflight_never_calls_provisioner(self):
        module = load_cli_module()
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            state = root / "state"
            ...
            with mock.patch.object(module, "approved_controlled_certification_substrate_binding", return_value=binding), mock.patch.object(module.subprocess, "run") as run:
                result = module.provision_approved_controlled_certification_substrate(args, state_dir=state, audit_dir=root)
            self.assertEqual(result["final_verdict"], "CONTROLLED_CERTIFICATION_SUBSTRATE_PREFLIGHT_READY")
    ...
    def test_one_user_existing_pool_reuse_defers_target_to_later_matrix_selection(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            binding = {"ok": True, "blockers": [], "decision_id": "cpsdec_test", "source_id": "controlled-source", "scope": {
                "profile": operator_execution.CONTROLLED_CERTIFICATION_SUBSTRATE_CT_M0F_ONE_USER_PROFILE,
                "target_total_certification_identities": 1,
                "max_new_certification_identities": 0,
                "identity_strategy": "REUSE_EXISTING_VALID_POOL",
                "campaign_stages": [1], "automatic_stage_progression": False,
            ...
            self.assertEqual(result["controlled_target"], "")
    ...
    def test_availability_baseline_reset_reuses_controlled_certification_gate_profile(self):
        module = load_cli_module()
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            state = root / "state"
    ...
    def test_availability_baseline_reset_reconciles_post_clearance_route_commit(self):
        module = load_cli_module()
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            state = root / "state"
            ...
            (state / "autoswitch-restore-barrier.json").write_text(
                json.dumps({
                    "packet_id": "pkt_reset",
                    "operation_id": "govdry_reset",
                    "allowed_users": ["10.7.0.100"],
                    "allowed_targets": ["vless"],
                    "created_at": "2026-07-31T06:11:51+00:00",
                }),
                encoding="utf-8",
            )
            (events / "switch-history.jsonl").write_text(
        ...
        self.assertEqual(
            reconciled["mode"],
            (
                "POST_CLEARANCE_ROUTE_COMMIT_RECONCILED_"
                "FROM_EXISTING_OWNERS"
            ),
        )
        self.assertTrue(reconciled["temporally_bound_switch"])
        self.assertFalse(reconciled["production_outcome_credit"])

    def test_restored_stage_receipt_recovery_reuses_existing_owners_only(self):
        module = load_cli_module()
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            state = root / "state"
            ...
            state.mkdir()
            events.mkdir()
            forward_packet = "pkt_forward"
            forward_operation = "govexec_forward"
            reset_packet = "pkt_reset"
            reset_operation = "govdry_reset"
            (state / "users.registry").write_text(
                (
                    "ip=10.7.0.100 current=vless enabled=1 "
                    "certification_user=1 certification_group=campaign-a\n"
            ...
            (state / "service-matrix-refresh-summary.json").write_text(
                json.dumps({
                    "updated": "2026-07-31T06:12:19+00:00",
                    "availability_first_standing_policy_action": {
                        "status": "STOP_SAFE",
                        "consumer_result": {
                            "stage": 1,
            ...
            with mock.patch.object(
                ...
                return_value={
                    ...
                    "replay_consumed": True,
                    "learning_consumed": True,
                    "feedback_id": "execfb_test",
                    "learning_record_id": "learn_test",
                },
            ), mock.patch.object(
                module.operator_execution,
                "validate_standing_delegated_operational_policy",
        ...
        self.assertEqual(recovered["stage"], 1)
        self.assertEqual(
    ...
    def test_l3_validation_exact_campaign_identity_remains_strict(self):
        module = load_cli_module()
        captured = {}
        ...
        request_index = captured["command"].index(
            "--controlled-certification-campaign-request-id"
        )
        self.assertEqual(
            captured["command"][request_index + 1],
            "cpsauth_exact",
        )
        self.assertIn(
            "--controlled-certification-campaign-target",
    ...
    def test_controlled_topology_binding_allows_same_campaign_continuation(self):
        module = load_cli_module()
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            state = root / "state"
            ...
            state.joinpath("users.registry").write_text(
                "ip=10.7.0.76 enabled=1 current=source "
                "certification_user=1 certification_group=t48\n"
            ...
            manifest = {
                ...
                "certification_group": "t48",
                "reservation_mode": (
                    "RENEW_AND_CONTINUE_SAME_CAMPAIGN_"
        ...
        self.assertTrue(binding["allowed"], binding)
        self.assertEqual(
            binding["manifest"]["reservation_mode"],
