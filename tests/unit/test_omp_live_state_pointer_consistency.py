...
class OmpLiveStatePointerConsistencyTest(unittest.TestCase):
    ...
    def test_06_current_stop_mismatch_fails(self):
        drift = self.omp.replace("Resolved current stop: `NONE`", "Resolved current stop: `UNSAFE_IMPLEMENTATION`", 1)
    ...
    def test_07_current_next_action_mismatch_fails(self):
        drift = self.omp.replace("Resolved current next action: `AEP_PHASE_6_PRODUCTION_CERTIFICATION_PREPARATION`", "Resolved current next action: `DIAGNOSE_BINDING`", 1)
    ...
    def test_14_cap_u01_complete_and_u07_waiting_use_live_state(self):
        ...
        self.assertIn("| `NONE` |", cps_result["sequence_position_1"])
    ...
    def test_18_truth_consistency_is_go_only_when_omp_agrees(self):
        ...
        drift = self.omp.replace("Resolved current stop: `NONE`", "Resolved current stop: `STOP_SAFE`", 1)
