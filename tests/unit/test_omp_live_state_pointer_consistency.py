...
class OmpLiveStatePointerConsistencyTest(unittest.TestCase):
    ...
    def test_07_current_next_action_mismatch_fails(self):
        drift = self.omp.replace("Resolved current next action: `AEP_PHASE_3_CERTIFIED_AUTONOMOUS_BEHAVIOUR_GAP_REGISTER`", "Resolved current next action: `DIAGNOSE_BINDING`", 1)
