...
class OmpLiveStatePointerConsistencyTest(unittest.TestCase):
    ...
    def test_06_current_stop_mismatch_fails(self):
        drift = self.omp.replace("Resolved current stop: `NATURAL_SCHEDULED_RUN`", "Resolved current stop: `UNSAFE_IMPLEMENTATION`", 1)
    ...
    def test_07_current_next_action_mismatch_fails(self):
        drift = self.omp.replace("Resolved current next action: `SAFE_DEPLOY_THEN_WAIT_POST_REPAIR_NATURAL_RUN`", "Resolved current next action: `DIAGNOSE_BINDING`", 1)
    ...
    def test_14_cap_u01_complete_and_u07_waiting_use_live_state(self):
        ...
        self.assertIn("| `NATURAL_SCHEDULED_RUN` |", cps_result["sequence_position_1"])
    ...
    def test_18_truth_consistency_is_go_only_when_omp_agrees(self):
        ...
        drift = self.omp.replace("Resolved current stop: `NATURAL_SCHEDULED_RUN`", "Resolved current stop: `STOP_SAFE`", 1)
