...
class OmpLiveStatePointerConsistencyTest(unittest.TestCase):
    ...
    def test_07_current_next_action_mismatch_fails(self):
        drift = self.omp.replace("Resolved current next action: `AEP_PHASE_2_ACCEPTANCE`", "Resolved current next action: `DIAGNOSE_BINDING`", 1)
    ...
    def test_08_latest_consumed_report_mismatch_fails(self):
        drift = self.omp.replace("docs/reports/engineering/2026-07-14_093039_program_execution_and_consumption_reconciliation.md", "docs/reports/engineering/stale.md", 1)
