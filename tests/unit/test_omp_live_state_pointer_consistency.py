...
class OmpLiveStatePointerConsistencyTest(unittest.TestCase):
    ...
    def test_07_current_next_action_mismatch_fails(self):
        drift = self.omp.replace("Resolved current next action: `EXECUTE_CAP_U07_LEARNING_OUTCOME_CONSUMPTION`", "Resolved current next action: `DIAGNOSE_BINDING`", 1)
    ...
    def test_14_cap_u01_complete_and_u07_frontier_use_live_state(self):
