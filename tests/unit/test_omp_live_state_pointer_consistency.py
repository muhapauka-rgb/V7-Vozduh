...
class OmpLiveStatePointerConsistencyTest(unittest.TestCase):
    ...
    def test_06_current_stop_mismatch_fails(self):
        drift = self.omp.replace("Resolved current stop: `ENGINEERING_AUTHORITY`", "Resolved current stop: `UNSAFE_IMPLEMENTATION`", 1)
    ...
    def test_07_current_next_action_mismatch_fails(self):
        drift = self.omp.replace("Resolved current next action: `APPROVE_SAFE_DEPLOY_THEN_ENABLE_POST_REPAIR_NATURAL_RUN`", "Resolved current next action: `DIAGNOSE_BINDING`", 1)
