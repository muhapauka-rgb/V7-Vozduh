...
class OperatorExecutionPacketTest(unittest.TestCase):
    ...
    def test_v3_standing_policy_auto_consumes_only_exact_pending_substrate_request(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            write_json(policy_path, {"authority_budget": {}})
            standing_request = build_standing_delegated_policy_authority_request(
                policy_generation_hash=sha256_file(policy_path),
                active_program="V7_SERVICE_FAILURE_AUTOMATION_EVOLUTION_PROGRAM_V1",
                max_users=48,
                include_availability_first=True,
                include_auto_substrate=True,
                now=now,
            )
            register_standing_delegated_policy_request(
                standing_request, audit_store=audit, now=now,
            )
            issued = issue_standing_delegated_policy_from_audit(
                policy_path,
                request_id=standing_request["request_id"],
                request_hash=standing_request["request_hash"],
                decision="APPROVE_STANDING_DELEGATED_OPERATIONAL_POLICY",
                audit_store=audit,
                actor_id="operator-standing-delegation",
                now=now,
            )
            ...
            request = build_controlled_certification_substrate_authority_request(
                ...
                source_id="controlled-source",
                current_pool_status={
                    "total_enabled_certification_users": 40,
                    "max_enabled_certification_users_on_one_active_source": 40,
                    "fingerprint": "f" * 64,
                    "registry_hashes": {
                        "users_registry": "a" * 64,
                        "egress_registry": "b" * 64,
                ...
                current_policy_contract_id=contract["contract_id"],
                current_policy_contract_hash=contract["contract_hash"],
                now=now + timedelta(minutes=1),
            )
            register_controlled_certification_substrate_authority_request(
                request, audit_store=audit, now=now + timedelta(minutes=1),
            )
            first = operator_execution.consume_controlled_certification_substrate_from_standing_policy(
                policy_path, audit_store=audit, now=now + timedelta(minutes=2),
            )
            second = operator_execution.consume_controlled_certification_substrate_from_standing_policy(
                policy_path, audit_store=audit, now=now + timedelta(minutes=3),
            )
            records = read_audit_records(audit)
        self.assertEqual(first["status"], "AUTO_CONSUMED_BY_EXISTING_STANDING_POLICY")
        self.assertEqual(second["status"], "ALREADY_CONSUMED_EXACT")
        self.assertTrue(first["coverage"]["ok"])
        self.assertFalse(first["forbidden_effects"]["runtime_apply"])
        decisions = [
            row for row in records
            if row.get("record_type")
            == "controlled_certification_substrate_authority_decision"
        ...
        self.assertEqual(
            decisions[0]["actor_provenance"]["actor_id"],
            operator_execution.STANDING_DELEGATED_SUBSTRATE_CONSUMER_ACTOR,
        )
